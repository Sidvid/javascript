...
const getData = (person) => {
  return `${person.slice(0, person.length - 1).join(", ")} and ${
    person[person.length - 1]
  } `;
};
function desGenerator (data){
    const newData = data.map((item)=>{
        return {
          ...item,
          description: `${item.title} is a 55 minute education podcast hosted by ${item.hosts.length >1 ? getData(item.hosts) : item.hosts[0]} `,
        };

    })
return newData
}
console.log(desGenerator(data))